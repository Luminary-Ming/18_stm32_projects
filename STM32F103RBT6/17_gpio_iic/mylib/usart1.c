...
// 通过 USART1 接收多个字节的数据 (可变长的)
/*
u8 *usart1_recv_data1111(void)
{
	u32 timeout = 0;  // 超时标志
	u16 buf_size = 64;  // 初始缓冲区大小
	u16 len = 0;
	u16 i = 0;
	u32 need_size = 0;
	
	u8 *temp_buf = calloc(1, buf_size);
	...
	while(USART_GetFlagStatus(USART1, USART_FLAG_RXNE) == SET)
	{
		...
		{
			buf_size += 64;
			u8 *new_buf = realloc(temp_buf, buf_size);
			if(!new_buf)
				break;
		...
		temp_buf[len++] = USART_ReceiveData(USART1);
	...
	for(i = 0; i < len; i++)
	{
		need_size += snprintf(NULL, 0, "%02X", temp_buf[i]);
	} 
	
	u8 *recv_buf = malloc(need_size + 1);
	if(!recv_buf)
	{
		free(temp_buf);
		return NULL;
...
/*
void usart1_free_recv_data(u8 *data)
{
	if(data) free(data);
}
*/
