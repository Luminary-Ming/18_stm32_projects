...
void SPI2_For_Nrf24l01(void)//初始化NRF所使用到的SPI接口的管脚
{
	...
	GPIO_SetBits(GPIOA, GPIO_Pin_5);//默认是高电平(低电平有效)
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//选择了50MHz输出速度
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOA, GPIO_Pin_4);//默认是低电平(既不发送也不接收)
...
void NRF_RX_Mode(u8 *rxAddr, u8 channel, u8 recvLen)
{
	...
	SPI_NRF_WriteReg(NRF_WRITE_REG+SETUP_AW, 0x03);//地址为5字节的
	SPI_NRF_WriteReg(NRF_WRITE_REG+RF_CH, channel); //设置RF通信频道
