...
{
	// AT+RST\r\n 重启无线模块
	usart2_send_data((u8 *)"AT+RST\r\n");
	delay_ms(2000);  // 重启之后延时两秒
	
	// AT+CWMODE=1\r\n 选择wifi模式
	ok_flag = 0;
	usart2_send_data((u8 *)"AT+CWMODE=1\r\n");  // 选择模式
	while(ok_flag == 0);  // 等待esp8266的OK
	
	// AT+CWJAP="SSID","password"\r\n 连接路由
	ok_flag = 0;
	usart2_send_data((u8 *)"AT+CWJAP=");
	usart2_send_byte('"');
	usart2_send_data((u8 *)ssid);
	usart2_send_byte('"');
	usart2_send_byte(',');
	usart2_send_byte('"');
	usart2_send_data((u8 *)passwd);
	usart2_send_byte('"');
	usart2_send_data((u8 *)"\r\n");
	while(ok_flag == 0);
	
	// AT+CIFSR\r\n 查询分配到的IP地址(必须要查一下,否则无法建立连接)
	usart2_send_data((u8 *)"AT+CIFSR\r\n");
}
...
void esp8266_connect(char *ip, char *port)  // 连接TCP server
{
	...
	ok_flag = 0;
	usart2_send_data((u8 *)"AT+CIPSTART=");
	usart2_send_byte('"');
	usart2_send_data((u8 *)"TCP");
	usart2_send_byte('"');
	usart2_send_byte(',');
	usart2_send_byte('"');
	usart2_send_data((u8 *)ip);
	usart2_send_byte('"');
	usart2_send_byte(',');
	usart2_send_data((u8 *)port);
	usart2_send_data((u8 *)"\r\n");
	while(ok_flag == 0);
}
...
{
	// AT+CIPCLOSE\r\n 断开TCP连接
	usart2_send_data((u8 *)"AT+CIPCLOSE\r\n");
...
void esp8266_send(char *msg, char *len)  // 发送数据
{
	// AT+CIPSEND=4\r\n 发送数据
	ok_flag = 0;
	usart2_send_data((u8 *)"AT+CIPSEND=");
	usart2_send_data((u8 *)len);
	usart2_send_data((u8 *)"\r\n");
